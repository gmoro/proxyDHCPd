...
class DhcpPacket(DhcpBasicPacket):
    def str(self):
        ...
        for opt in  ['htype','hlen','hops','xid','secs','flags',
            ...
            elif DhcpFieldsTypes[opt] == "int2" : result = str(data[0]*256+data[1]) if len(data) >= 2 else str(data[0])
            elif DhcpFieldsTypes[opt] == "int4" : result = str(ipv4(data).int()) if len(data) >= 4 else ""
        ...
        for opt in self.options_data.keys():
            ...
            if not data:
                pass
            elif opt=='dhcp_message_type' : result = DhcpFieldsName['dhcp_message_type'].get(str(data[0]), str(data[0]))
            elif DhcpOptionsTypes[optnum] == "char" : result = str(data[0])
            elif DhcpOptionsTypes[optnum] == "16-bits" : result = str(data[0]*256+data[1]) if len(data) >= 2 else str(data[0])
            elif DhcpOptionsTypes[optnum] == "32-bits" : result = str(ipv4(data).int()) if len(data) >= 4 else ""
